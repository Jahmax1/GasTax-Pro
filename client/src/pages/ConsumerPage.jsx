    ...
    <div className="container mx-auto p-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="bg-gray-800 p-4 rounded-lg card-hover">
          <h2 className="text-xl font-semibold mb-2">Verify Receipt</h2>
          <input
            type="text"
            value={receiptId}
            onChange={(e) => setReceiptId(e.target.value)}
            placeholder="Enter Receipt ID (e.g., receipt-1)"
            className="w-full p-2 bg-gray-700 rounded mb-2 text-white placeholder-gray-400"
          />
          <button
            onClick={handleVerify}
            className="bg-green-600 hover:bg-green-700 p-2 rounded w-full transition-colors"
          >
            Verify
          </button>
          {receipt && (
